 ...
 */
var createWebsite = function(siteTemplateName, callback) {

  var config = {
    websiteFolder : process.cwd(),
    siteTemplateFolder : pt.resolve(__dirname, '../..', siteTemplateName ? TEMPLATE_SITE_FOLDER + siteTemplateName : DEFAULT_SITE),
...
var readTemplateDir = function (config, path, endCallback) {
  fs.readdir(path, function(error, files){
    ...
    if (error) {
      endCallback(new Error ("Impossible to find the site template in the folder : " + path));
      return;
    }
