...
var generateHtaccess = function (config, callback) {
  ...
  if (config.cloaks && config.cloaks.rules) {
...
var generateCloakingRules = function (config) {
  ...
  config.cloaks.rules.forEach(function(rule) {

      if (rule.method == "userAgent") {
          var userAgentRules = addRule(rule, "RewriteCond %{HTTP_USER_AGENT}");
          Array.prototype.push.apply(cloakingRules, userAgentRules);
      }

      if (rule.method == "host") {
        var hostRules = addRule(rule, "RewriteCond %{HTTP_HOST}");
        Array.prototype.push.apply(cloakingRules, hostRules);
      }
...
var addRule = function (rule, conditionPrefix) {

  var htAccessRules = [];

  for (i=0; i< rule.conditions.length; i++) {
    var htAccessRule = conditionPrefix + " " + rule.conditions[i];
    if (rule.conditions.length > 1 && i < rule.conditions.length -1 ) {
      htAccessRule += " [OR]";
    }
    htAccessRules.push(htAccessRule);
  }

  for (i=0; i<rule.rewrites.length; i++) {
    var htAccessRule = "RewriteRule " + rule.rewrites[i].from + " " +  rule.rewrites[i].to +  " [R=301]";
    htAccessRules.push(htAccessRule);
  }
