  ...
  config.cloaks.forEach(function(rule) {
      ...
      if (rule.conditions) {
            ...
            if (rule.conditions.userAgents) {
              var userAgentRules = addConditions(rule.conditions.userAgents, "RewriteCond %{HTTP_USER_AGENT}");
            ...
            if (rule.conditions.ips) {
              ...
              Array.prototype.push.apply(cloakingRules, ipRules);
            }

      }

      rule.rewrites.forEach(function(rewrite) {
        // Content replacement is not supporting in the htaccess, only redirect 301
        var htAccessRule = "RewriteRule " + rewrite.from + " " +  rewrite.to +  " [R=301]";
...
var addConditions = function (conditions, conditionPrefix) {
