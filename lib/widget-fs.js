...
var renderWidgets = function(params, endCallback) {
    ...
    async.each(params.config.widgets,
      function(regionWidgets, callback) {
          log.info("Render HTML for widgets : " + regionWidgets.widgets + " for region : " + regionWidgets.region);

          renderWidgetForRegion(params, regionWidgets, function(error, regionHTML){
              if(error) {
                callback(error);
                return;
              }
...
var renderWidgetForRegion = function(params, regionWidgets, endCallback) {
    var regionHTML = [];

    async.each(regionWidgets.widgets,
      function(widgetName, callback) {
          renderHTML(params, widgetName, function(error, htmlCode){
            if (error) {
              callback(error);
              return;
            }
            regionHTML.push(htmlCode);
            callback();
          });
...
var renderHTML = function(params, widgetName, callback) {

  var widgetFile = params.config.dirname + "/" + params.config.widgetFolder + "/" + widgetName;
