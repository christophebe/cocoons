...
CloakingRules.prototype.applyCloakingRule = function (rules, req, res, endCallback) {
      ...
      function(rule){
          ...
          if (rule) {
            ...
            var newUrl = that.getToUrl(req.url, rewriter.to);

            if (rewriter.type == "replace") {
              debug(req, "Cloaking found (content replace) for : " + req.url + " by " + newUrl);
              endCallback(null, newUrl);
              return;
            }


            if (rewriter.type == "301") {
              debug(req, "Redirect 301 found for : " + req.url + " => " + newUrl);
              res.redirect(301, that.getToUrl(req.url, newUrl));
              endCallback(null, "");
              return;
            }

            if (rewriter.type == "302") {
              debug(req, "Redirect 302 found for : " + req.url + " => " + newUrl);
              res.redirect(302, that.getToUrl(req.url, newUrl));
              endCallback(null, "");
              return;
            }
            ...
            if (rewriter.type == "404") {
              debug(req, "Cloaking found (404) for : " + newUrl);
            ...
            if (rewriter.type == "500") {
              debug(req, "Cloaking found (500) for : " + newUrl);
...
CloakingRules.prototype.getToUrl = function (url, to) {
