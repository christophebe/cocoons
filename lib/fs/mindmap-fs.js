...
//    in the cocoons.json
//  - Partial generation of the site architcture based on url/path
// => ask a question to the user or set a specific config
//
// -----------------------------------------------------

// The template used to create the markdown content for each pages
var MD_TEMPLATE_FILE = pt.resolve(__dirname, "..", "mindmap/markdown.jade");
...
function Mindmap (config) {
    ...
    srcFolder = process.cwd() + "/" +  config.source;
    log.info("The site structure will be generated into : " + srcFolder);
...
Mindmap.prototype.generateFromMindmap = function(mindmapFilePath, endCallback) {
    ...
    async.waterfall([
        ...
        function(callback){
            util.compileJade(MD_TEMPLATE_FILE, function(error, compiledTemplate) {
              if (error) {
                callback(error);
                return;
              }
...
Mindmap.prototype.generatePage = function(page, endCallback) {
    ...
    async.series([
        function(callback){
            if (page.children)
            {
                log.info("Create directory : " + srcFolder + page.path);
                mkpath(srcFolder + page.path, function (error) {
                    callback(error);
                });
            }
            else {
...
Mindmap.prototype.getFileName = function (page, extension) {
  ...
  if (page.path == "") {
    file = srcFolder + "/index" + extension;
  }
  else {
    file = srcFolder + page.path + extension;
  }
...
Mindmap.prototype.createMainMenu = function (homePage, callback) {
  var cocoonsFile = process.cwd() + "/cocoons.json";
